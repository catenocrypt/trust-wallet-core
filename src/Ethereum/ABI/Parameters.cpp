...
int ParamSet::addParam(ParamBase* param) {
...
void ParamSet::addParams(std::vector<ParamBase*> params) {
    for (auto p: params) addParam(p);
}

bool ParamSet::getParam(int paramIndex, ParamBase** param_out) const {
...
ParamBase* ParamSet::getParamUnsafe(int paramIndex) const {
    if (_params.size() == 0) {
